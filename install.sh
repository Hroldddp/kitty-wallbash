...
SDDM_SYSTEMD_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user"
...
install_sddm() {
    ...
    if [ -f "$CACHE_DIR/wall.dcol" ] && [ -f "$CACHE_DIR/wall.set.png" ]; then
        ...
        print_skip "wall.dcol not ready yet — colors will apply on next wallpaper change"
    else
...
EOF
    ...
    # Install systemd path unit: watches wall.set for changes, triggers sddm.sh
    mkdir -p "$SDDM_SYSTEMD_DIR"
    local src_path="./config/sddm-watch.path"
    local src_svc="./config/sddm-watch.service"
    local dst_path="$SDDM_SYSTEMD_DIR/sddm-watch.path"
    local dst_svc="$SDDM_SYSTEMD_DIR/sddm-watch.service"
    cp "$src_path" "$dst_path"
    cp "$src_svc" "$dst_svc"
    systemctl --user daemon-reload 2>/dev/null || true
    systemctl --user enable --now sddm-watch.path 2>/dev/null && \
        print_ok "SDDM systemd path watcher enabled (triggers on every wallpaper/theme change)" || \
        print_warn "Could not enable systemd path watcher (non-systemd session?)"
}
...
verify() {
    ...
    if systemctl --user is-enabled sddm-watch.path &>/dev/null; then
        print_ok "SDDM systemd path watcher enabled"
    else
